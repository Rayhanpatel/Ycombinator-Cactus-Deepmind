...
class CactusEngine:
    ...
    def complete_multimodal(
        ...
        system_prompt: str = "You are a helpful VoiceSight field inspector. Analyze the audio and image to log findings.",
        tools_json: Optional[str] = None,
    ) -> dict:
        """
        Run a multimodal completion with raw audio and/or image input.
        Gemma 4 natively processes both in a single pass.

        Args:
            pcm_data: Raw PCM audio as a list of int16 samples (16kHz, mono).
            base64_image: Base64 encoded JPEG string of the camera frame.
        """
        if not self._initialized:
            return {"success": False, "error": "Engine not initialized"}

        # Build OpenAI-style content array for multimodal
        user_content = []
        if base64_image:
            # We prefix it properly so the Cactus rust/cpp parser knows it's an image
            user_content.append({
                "type": "image_url",
        ...
        if pcm_data:
            # The Cactus tokenizer expects the <audio> string inline for PCM correlation
            user_content.append({"type": "text", "text": "<audio>"})

        # Fallback if both are empty
        if not user_content:
            user_content.append({"type": "text", "text": "Please analyze."})
        ...
        if result.get("success"):
            logger.debug(
                f"📷🎙️ Multimodal TTFT={result.get('time_to_first_token_ms', '?')}ms | "
                f"Decode={result.get('decode_tps', '?')} tok/s"
            )
