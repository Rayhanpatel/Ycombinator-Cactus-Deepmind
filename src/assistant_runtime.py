...
class AssistantSession:
    ...
    def _strip_history_tool_messages(self) -> None:
        ...
        self.messages = [message for message in self.messages if message.get("role") != "tool"]
        dropped = before - len(self.messages)
        if dropped:
            logger.debug("Dropped %s tool message(s) from history", dropped)
        ...
        *,
        ...
        async with self._turn_lock:
            ...
            try:
                ...
                self.session._strip_history_tool_messages()
